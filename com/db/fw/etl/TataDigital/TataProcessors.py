...
class TDBaseEventParseProcessor(BaseProcessor):
...
class MasterProcessor(Task):
    ...
    def execute(self):
        ...
        payloadParsedDf = payloadDf.filter("dataSize > 0").withColumn("data", explode(payloadDf["data"]))

        payloadMasterDf = payloadParsedDf.selectExpr("event_type", "cast(data AS String) AS data",
...
class HistoryWithDQCheckProcessor(Task):
    def __init__(self, name, type):
        Task.__init__(self, name, type)

    def execute(self):
        payloadMasterDf = self.get_input_dataframe()


        payloadHistoryDf = payloadMasterDf.selectExpr("data.merchantOrderId AS source_order_detail_id",
                    ...
                    "data.payOrderId").repartition(payloadMasterDf["customer_hash"])

        payloadHistoryDf.createOrReplaceTempView("payloadHistoryView")
